...
def cells_obj_to_states_arr(cells):
    ...
    row = 0
    for pos in cells:
        raw_state = str(cells[pos].state) if cells[pos].state != 0 else "0"
...
states_arr = cells_obj_to_states_arr(cells)

str_arr_to_int_arr(states_arr)

solve(states_arr)
flatten_states = sum(states_arr, [])
print(flatten_states)

for i,pos in enumerate(cells):
    cells[pos].state = flatten_states[i]
    cells[pos].clickable= False
